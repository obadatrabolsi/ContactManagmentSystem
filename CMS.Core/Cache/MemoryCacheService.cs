...
{
    internal class MemoryCacheService : ICacheManager
    {
        #region Ctor & Props
        ...
        /// <summary>
        /// DefaultCacheTime in minutes
        /// </summary>
        private readonly int _defaultCacheTime;

        /// <summary>
        /// Represent whether to enable or disable the caching
        /// </summary>
        private readonly bool _enableCahcing;
            ...
            CacheSettings cacheSettings)
        {
            ...
            _enableCahcing = cacheSettings.EnableCaching;
            _defaultCacheTime = cacheSettings.DefaultCacheTime;

            if (_defaultCacheTime <= 0)
                _defaultCacheTime = 1440; // 1 day
        }
        ...
        /// <param name="cacheTime">Cache time in minutes; pass 0 to do not cache; pass null to use the default time</param>
        /// <returns>The cached value associated with the specified key</returns>
        public virtual async Task<T> Get<T>(string key, Func<Task<T>> acquire, int? cacheTime = null)
        {
            key = FormatKey(key);
        ...
        {
            return _allKeys.ToDictionary(x => x.Key, x => _cache.Get(x.Key));
        }
        ...
        {
            if (data != null && _enableCahcing)
            {
                key = FormatKey(key);

                _cache.Set(AddKey(key), data, GetMemoryCacheEntryOptions(TimeSpan.FromMinutes(cacheTime ?? _defaultCacheTime)));
            }
        }
        ...
        public virtual void ClearAll()
        {
            ...
            var keys = _allKeys.Select(x => x.Key).ToList();
